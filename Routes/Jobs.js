...
const indeedNoLoc = require('../data-resources/indeed').customNoLoc
const authenticJobsNoLoc = require('../data-resources/authenticjobs').customNoLoc
...
router.post('/NoLocPaginated', (req, res) => {
  const keywords = req.body.keywords.map(word => new RegExp(`${word.replace('-', '(-|\\s)?')}`, 'i'))

  const $or = keywords.map($regex => ({ title: { $regex } }))
    .concat(keywords.map($regex => ({ description: { $regex } })))

  jobs.find({ $or }, 'title company location datepost URL', { sort: '-datepost' })
    .then(docs => {
      const startPoint = Math.floor((req.body.page - 1) * 50)
  ...
  jobs.find({ $or }, 'title company location datepost URL', { sort: '-datepost' })
    .then(docs => {
      ...
      if (length < 25) {
        ...
        Promise.all([
          indeedNoLoc(req.body.keywords),
          authenticJobsNoLoc(req.body.keywords)
        ])
          .then(newData => {
            const found_jobs = [].concat(...newData)
            found_jobs.sort((a, b) => b.datepost - a.datepost)
